...
export async function transcribeAudio(audioAsBase64: string, mimeType: string) {
  const response = await gemini.models.generateContent({
    ...
    contents: [
      {
        text: 'Transcreva o áudio para português do Brasil. Seja preciso e natural na transcrição. Mantenha a pontuação adequada e divida o texto em parágrafos se necessário',
      },
      {
        inlineData: {
          mimeType,
...
export async function generateAnswer(
  ...
  INSTRUÇÕES:
    ...
    - Se a resposta não for encontrada no contexto, apenas responda não possui informações suficientes para responder;
    - Seja objetivo;
